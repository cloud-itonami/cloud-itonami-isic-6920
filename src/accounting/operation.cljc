(ns accounting.operation
  "OperationActor -- one accounting/auditing operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (Ledger-LLM) is sealed into a single node (:advise); its proposal is
  ALWAYS routed through the Audit Independence Governor (:govern) and
  ...
  One graph run = one accounting/auditing operation (intake -> advise
  ...
  decision to a human operator (the licensed accountant/auditor). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:audit-opinion/issue`/`:tax-filing/submit` ALWAYS reach
  this node when the governor is clean -- see `accounting.phase`."
...
(defn build
  ...
  opts:
    :advisor      -- an `accounting.ledgerllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (ledgerllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Ledger-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (ledgerllm/-advise advisor store request)]
            {:proposal p :audit [(ledgerllm/trace request p)]})))

      ;; Audit Independence Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
